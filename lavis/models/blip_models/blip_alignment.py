...
@registry.register_model("blip_alignment")
class BlipAlignment(BlipBase):
    """
    ...
    def score(self, samples, match_head="itm"):
        image = samples["image"]
        ...
        if match_head == "itm":
            ...
            itm_scores = torch.nn.functional.softmax(itm_output, dim=1)[:,1] * 4 + 1
            return itm_scores
    ...
    def forward(self, samples, match_head="itm"):
        ...
        if match_head == "itm":
            ...
            itm_output = self.itm_head(output.last_hidden_state[:, 0, :])
            itm_scores = F.softmax(itm_output, dim=1)[:,1] * 4 + 1
            loss = torch.mean(torch.abs(itm_scores - score))
            return BlipOutput(loss=loss, loss_itm=loss)
